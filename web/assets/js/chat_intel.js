function showQuestion(question_id) {
  ...
  xhr.open('GET', 'functions/action.php?action=showQuestion&question_id=' + question_id + '&case_id=' + case_id, true);
  xhr.send();
...
function searchQuestion() {
  ...
  $('#sendArea').val('');
  var xhr = new XMLHttpRequest();
  xhr.open('GET', 'functions/search_question.php?product_id=' + product_id + '&case_id=' + case_id + '&input=' + input, true);
  xhr.send();
  xhr.onreadystatechange = function () {
    if (xhr.readyState == 4 && xhr.status == 200) {
      document.getElementById("chatting").innerHTML = xhr.responseText;
